...
int menu(const HANDLE hout)
{
	...
	while (1) {
		...
		cout << "5.用I、J、K、L键控制上下左右(按左箭头键不向下移动，大小写均可，边界停止" << endl;
		cout << "6.用I、J、K、L键控制上下左右(按左箭头键不向下移动，大小写均可，边界回绕" << endl;
		cout << "0.退出" << endl << "[请选择0-6]: ";
...
void move_by_arrow(const HANDLE hout, int tag)
{
	cls(hout);
	init_border(hout);
