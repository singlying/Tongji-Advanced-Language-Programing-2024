...
//对学号进行排序
void bubble_sort_num(char name[][9], char num[][8], int score[], int order)
{
	char temp_char[9];
	int temp_int;
	...
	for (int i = 0; i < SUM - 1; i++) {
		for (int j = 0; j < SUM - i - 1; j++) {
			if (order == ASC ? strcmp(num[j], num[j + 1]) == 1 : strcmp(num[j], num[j + 1]) == -1) {
...
int main()
{
	...
	bubble_sort_num(name, num, score, DESC);
